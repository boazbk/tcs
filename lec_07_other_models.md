...
The "granddaddy" of all models of computation is the _Turing Machine_, and indeed this is the model that is commonly used  as the standard model in most textbooks.^[This definitional choice does not make much difference since, as we will show, NAND++/NAND<< programs are equivalent to Turing machines in their computing power.]
Turing machines where defined in 1936 by Alan Turing in an attempt to formally capture all the functions that can be computed by human "computers" that follow a well-defined set of rules, such as the standard algorithms for addition or multiplication.
...
For simplicity we can think of this scratch paper as a one dimensional piece of graph paper, where in each box  we can write  a single symbol.
...
We say that the Turing machine $M$ computes a (partial) function $F:\{0,1\}^* \rightarrow \{0,1\}^*$ if for every $x\in\bits^*$ on which $F$ is defined, the result of the following process is $F(x)$:
>
...
That is, we can think of a computation of a Turing Machine $M$ with $k$ states and alphabet $\Sigma$ on input $x\in \{0,1\}^*$ as follows:

* Initially the machine is at state $0$ (known as the "starting state") and the tape is initalized to $\triangleright,x_1,\ldots,x_n,\varnothing,\varnothing,\ldots$
...
* At each step, the machine reads the symbol $\sigma = T[i]$ that is in the $i^{th}$ location of the tape, and based on this symbol and its state $s$ decides on:
    ...
    * What is going to be the new state $s \in \{1,\ldots, k\}$
...
~~~~ { .go .numberLines }
tape_0 := ">" // tape is an array with the alphabet Sigma
k := 0
...
Using this, we can easily simulate an execution of the program $P$ using a Turing machine $M$, updating the internal state based on the actions of $P$ and updating  the location of the head (with an increase corresponding to a right move and a decrease corresponding to a left move) based on the value of the variable `idxdecreasing`.
...
(One shudders to think what name would be for the model where the words are even longer, such as $\sqrt{n}$.)
...
We will not show all the details but it is not hard to show that this model is equivalent to NAND<< programs.
...
Any function computed by a standard programming language such as `C`, `Java`, `Python`, `Pascal`, `Fortran` etc.. can be computed by a NAND++ program much in the way that we showed how to write an interpreter for Python programs in NAND.
...
We can also translate NAND++ programs to such programming languages, and  the website [http://nandpl.org](http://nandpl.org) contains both  code to evaluate NAND++ programs.^[TODO: add some comment about unbounded memory.]
...
Interestingly, while Turing Machines are not (and have never been) actually used for  performing computation, the $\lambda$ calculus has inspired functional programming languages such as LISP and ML, and so indirectly, the development of many other programming languages as well.
...
We can use this approach in general to achieve the effect of functions with more than one input and so we will use the shorthand $\lambda x,y. e$ for $\lambda x. (\lambda y. e)$.^[This technique of simulating multiple-input functions with single-input functions is known as [Currying](https://en.wikipedia.org/wiki/Currying) and is named after the logician [Haskell Curry](https://en.wikipedia.org/wiki/Haskell_Curry).]

Because function invocation are so common in the $\lambda$-calculus, and because (as anyone familiar with LISP knows) the $\lambda$-calculus does not suffer from a shortage of parenthesis, we allow dropping the parenthesis in a function invocation, and so if $f$ is a $\lambda$ expression and $z$ is some other expression, then we can write  $fz$ instead of $f(z)$ for the expression corresponding to invoking $f$ on $z$.^[When using  identifiers with multiple letters for $\lambda$ expressions,  we'll separate them with spaces or commas.]
That is, if $f$ has the form $\lambda x.e$ then $fz$ corresponds to $e[x \rightarrow z]$.
...
We  can invoke one $\lambda$ expression on another.
...
__Defining bits and strings in the $\lambda$ calculus.__
...
More generally we identify a string $i \in \{0,1\}^n$ (which can also be thought of as a number in $[2^n]$) with the function $\lambda x_0,\ldots,x_{2^n-1}. x_i$.
If we let $C = \lambda f,g.fgg$ then for a string $x$ and a bit $b$, $Cbx$ corresponds to the string $xb$: can you see why?
...
__$\lambda$-computable functions:__
...
__Simulating NAND++ programs using $\lambda$ expressions.__
The "if" direction of [lambdaturing-thm](){.ref} is less obvious, and we won't show the full details here.
For starters, we can verify that the following expression corresponds to the NOT operation:

$$
\lambda f,x,y.f(y,x)
$$

and the following expression corresponds to the AND operation:

$$
\lambda f,g.fgf
$$

and hence we can combine those to simulate any NAND program $P$ by a $\lambda$ expression $\phi_P$.


To handle   loops (i.e., NAND++ programs, as opposed to NAND programs), we need to use _recursion_.
...
$$
f = \lambda n. ITE(n==0,1,f(n-1))
$$
where $ITE(a,b,c)$ is the "If-Then-Else" function that outputs $b$ if $a=1$ and $c=0$, $n=0$ is the function that outputs $1$ if $n=0$ and $0$ otherwise, and $n-1$ is defined in the natural way.

Instead we can also think of $f$ as the unique _fixed point_ of the following function $F$:

$$
F = \lambda f. (\lambda n. ITE(n==0,1,f(n-1)))
$$

That is, factorial function $f$ is the only function that satisfies $Ff= f$.
Note that $F$ itself is _not_ a recursive function.
Hence we could do without  recursion in the $\lambda$ calculus if only we had some $\lambda$-expression $Y$ such that $YF$ always evaluates to a fixed point of $F$.
It turns out that we have precisely such an expression
$$
Y = \lambda f. (\lambda x. f (x x)) (\lambda y. f (y y))
$$
this is known as the [Y combinator](https://en.wikipedia.org/wiki/Fixed-point_combinator#Fixed_point_combinators_in_lambda_calculus).
...
Using the $Y$ combinator we can implement recursion in the $\lambda$-calculus, and hence loops. This can be used to complete the "if" direction of [lambdaturing-thm](){.ref}.
