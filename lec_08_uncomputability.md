...
Suppose otherwise that $x(x) \neq 1$ (and hence $F^*(x)=0$).
In this case there are two possibilities:

* __Case 1:__ The machine described by $x$ does not halt on the input $x$. In this case, $HALT(x,x)=0$. Since we assume that $M_{HALT}$ computes $HALT$ it means that on input $x,x$, the machine $M_{HALT}$ must halt and output the value $0$. This means that [halttof](){.ref} will set $z=0$ and output $1$.


* __Case 2:__ The machine described by $x$ halts on the input $x$ and outputs some $y' \neq 1$. In this case, since $HALT(x,x)=1$, under our assumptions, [halttof](){.ref} will set $y=y' \neq 1$ and so output $1$.
