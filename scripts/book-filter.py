...
This is a pandoc filter (based on the panflute package) that I am using 
to transform the markdown source of my book into latex->pdf, html, and ms word.
It probably won't work for you as is, but you would need to tailor it to your needs.
It is provided here just in case it is useful for people attempting similar projects.
"""
...
def prepare(doc):
    ...
    logfilename = os.path.join(logdir,os.path.basename(file)+".log") if file else "book-filter.log"
    ...
    D = doc.get_metadata("latexsectionheaders", {1:"chapter", 2:"section", 3:"subsection", 4:"subsubsection", 5:"paragraph", 6:"subparagraph"})
    doc.latex_headers = { int(k):D[k] for k in D }
    logstring("Latex Headers: "+ str(doc.latex_headers),doc)
    doc.label_classes = doc.get_metadata("latexsectionheaders", {"solvedex":"Solved Exercise", "bigidea": "Big Idea"})

    doc.handlers = [h_paragraph,  h_csvtable, h_add_search, h_block_header, h_link_ref , h_latex_headers, h_latex_div, h_code_block, h_code_inline_draft , h_latex_image, h_latex_cite, h_html_footnote, h_html_header, h_emph, h_math]
...
def finalize(doc):
    ...
    if doc.format == "html":
        ...
        if file and title and searchindex:
            ...
            updateindex(searchindex,file+".html",
            title,text,doc)


    if doc.format == "html" and doc.footnotecontents:
        logstring("inserting "+str(doc.footnotecontents),doc)
        L = [pf.RawBlock("<ol>",format="html")]+doc.footnotecontents + [pf.RawBlock("</ol>",format="html")]
...
def tochtml(toc,title=""):
    ...
    def depth(key): return key.count(".")
    d = 0
    HTML = dedent(fr"""
        <ul class="summary">
    ...
    for k in keys:
        ...
        entry = ""
        if depth(k)==d and not first: entry = r"</li>"
        first = False
        if depth(k)>d:  entry = r"<ul>"+r"<li><ul>"*(depth(k)-d-1)
        if depth(k)<d:  entry  = r"</li>"+r"</ul></li>"*(d-depth(k))
        entry += dedent(fr"""
...
def get_table_width(options,doc):
    ...
    try:
        table_width = float(fractions.Fraction(
            (options.get('table-width', 1.0))),doc)
...
def parse_alignment(alignment_string, number_of_columns,doc):
    ...
    try:
        ...
        logstring(
            "pantable: alignment string is too long, truncated instead.",doc)
    ...
    try:
        ...
        logstring(
            "pantable: alignment: invalid character found, default is used instead.",doc)
        return None
...
def h_csvtable(e, doc):
    f = pf.yaml_filter(element = e, doc = doc, tag="table", function=convert2table,  strict_yaml=True)
    if f: logstring("Found table " + str(f),doc)
    return f
...
def h_html_footnote(e,doc):
    ...
    doc.footnotecontents += [pf.RawBlock(htmlcontent_before,format="html")] + [conts] + [pf.RawBlock(htmlcontent_after,format="html")]
...
def h_block_header(e,doc):
    ...
    h = e.content[0]
    f = pf.Div(*e.content[1:],identifier = h.identifier, classes = h.classes, attributes = h.attributes)
    e =  action(f,doc)
...
def h_latex_headers(e,doc):
    ...
    if not isinstance(e, pf.Header): return None
    if isinstance(e.parent,pf.BlockQuote): return None
    ...
    h = e.level
    if not h in doc.latex_headers: raise Exception(f"Invalid header level {h} in {e}")
    header = "\\"+doc.latex_headers[h]
    return pf.Para(pf.RawInline(header+"{",format="latex"),*e.content,pf.RawInline("}"+labeltext,format="latex"))
...
def h_code_block(e,doc):
    ...
    if not isinstance(e, pf.CodeBlock) or doc.format != 'latex': return None
    before = "\\begin{code}\n"
    after  = "\n\\end{code}\n"
...
def h_latex_image(e,doc):
    """
    Handle images with automatic sclaing.
    Image classes: (for tufte)
    full : full page  - figure*
    margin: margin figure - marginfigure
    """
    if not isinstance(e,pf.Para) or doc.format != 'latex': return None
    if not len(e.content)==1: return None
    if not isinstance(e.content[0],pf.Image): return None
    img = e.content[0]
    capoffset = ""
    figoffset = ""
    preamble = ""
    ...
    if label: end = fr"\label{{{label}}}"+end
...
def h_code_inline_draft(e,doc):
    ...
    if not isinstance(e, pf.Code)  or doc.format != 'latex': return None
